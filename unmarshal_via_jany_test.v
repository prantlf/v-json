...
fn test_unmarshal_null_to_scalar() {
	unmarshal_via_jany[int]('null') or {
...
fn test_unmarshal_int() {
	r := unmarshal_via_jany[int]('1')!
	assert r == 1
}

fn test_unmarshal_u8() {
	r := unmarshal_via_jany[u8]('1')!
	assert r == 1
}

fn test_unmarshal_u16() {
	r := unmarshal_via_jany[u16]('1')!
	assert r == 1
}

fn test_unmarshal_u32() {
	r := unmarshal_via_jany[u32]('1')!
	assert r == 1
}

fn test_unmarshal_u64() {
	r := unmarshal_via_jany[u64]('1')!
	assert r == 1
}
...
fn test_unmarshal_f32() {
	r := unmarshal_via_jany[f32]('1.2')!
	assert r == 1.2
}

fn test_unmarshal_f64() {
	r := unmarshal_via_jany[f64]('1.2')!
	assert r == 1.2
}

fn test_unmarshal_bool() {
	r := unmarshal_via_jany[bool]('true')!
	assert r == true
}

fn test_unmarshal_string() {
	r := unmarshal_via_jany[string]('"a"')!
	assert r == 'a'
}
...
fn test_unmarshal_enum_num() {
	r := unmarshal_via_jany[Human]('1')!
	assert r == .woman
}

fn test_unmarshal_enum_nam() {
	r := unmarshal_via_jany[Human]('"woman"')!
	assert r == .woman
}
...
fn test_unmarshal_empty_input() {
	unmarshal_via_jany[Empty]('null') or {
...
fn test_unmarshal_primitive_types() {
	...
	r := unmarshal_via_jany[PrimitiveTypes](input)!
	assert r.h == .woman
}
...
// 		'bool':   Any(true)
// 	})
// 	r := unmarshal_via_jany[OptionalTypes](input)!
// 	assert r.h? == .woman
// 	assert r.u8? == 1
// 	assert r.u16? == 2
// 	assert r.u32? == 3
...
// 		'int': Any(null)
// 	})
// 	r := unmarshal_via_jany[PrimitiveNullType](input) or {
...
// 		'int': Any(null)
// 	})
// 	r := unmarshal_via_jany[OptionalNullType](input)!
// 	r.int or { return }
...
// 		'int': Any([Any(f64(1))])
// 	})
// 	r := unmarshal_via_jany[OptionalArray](input)!
// 	assert r.int?.len == 1
// 	assert r.int?[0] == 1
// }
...
// 		'int': Any([Any(f64(1))])
// 	})
// 	mut r := unmarshal_via_jany[ArrayOfOptions](input)!
// 	assert r.int.len == 1
// 	first := r.int[0]
// 	assert first? == 1
// }
...
// 		'arr': Any([Any(f64(1))])
// 	})
// 	r := unmarshal_via_jany[ArrayInStruct](input)!
// 	assert r.arr.len == 1
// 	assert r.arr[0] == 1
// }
...
fn test_unmarshal_struct_in_struct() {
	r := unmarshal_via_jany[OuterStruct]('{"inner":{"val":1}}')!
...
// 		})
// 	})
// 	r := unmarshal_via_jany[OuterStruct](input)!
// 	assert r.inner.val == 1
// }
...
fn test_attributes() {
	...
	r := unmarshal_via_jany_opt[Attributes](input, opts)!
	assert r.int == 1
	assert r.bool == false
	assert r.string == ''
	assert r.f64 == 2.3
